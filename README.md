# ⚡ Neon Survivors ⚡

Um jogo estilo Vampire Survivors feito em HTML/CSS/JavaScript puro. Sobreviva às hordas de inimigos, colete XP, evolua seus poderes e derrote os bosses!

## 🎮 Como Jogar

- **Jogar online:** <a href="https://dliedke.github.io/NeonSurvivors/neon-survivors.html" target="_blank" rel="noopener noreferrer">Abrir Neon Survivors</a>

Abra o arquivo `neon-survivors.html` em qualquer navegador moderno. Não requer instalação!

### Controles

| Ação | Teclado | Controle |
|------|---------|----------|
| Mover | `WASD` ou `Setas` | Analógico esquerdo |
| Tela cheia | `F` ou `F11` | - |
| Mais inimigos | `+` | 🔼 Triângulo |
| Menos inimigos | `-` | 🟥 Quadrado |
| Selecionar/Confirmar | - | 🅰️ Botão A |
| Navegar upgrades | - | ⬅️ ➡️ D-pad |

## ✨ Features

- 🎯 **Ataque automático** - Mira nos inimigos mais próximos
- ⬆️ **9 upgrades** com níveis - Dano, Cadência, Projéteis, Velocidade, Vida, Ímã, Penetração, Regeneração, Projétil Rápido
- 👹 **Bosses** - Aparecem a cada 45 segundos, ficam mais fortes a cada vez
- 💎 **Tesouros especiais** - 6 super poderes temporários (Explosão, Velocidade, Escudo, Cura, Fúria, Projéteis)
- 🔥 **Sistema de Combo** - Mate inimigos de perto para multiplicar XP (até 10x!)
- 🌊 **Sistema de Waves** - Complete waves para ganhar vida extra
- 🎛️ **Dificuldade ajustável** - Controle a quantidade de inimigos em tempo real
- 🔊 **Sons procedurais** - Efeitos sonoros gerados com Web Audio API
- 💥 **Screen shake** - Feedback visual ao tomar dano
- 📺 **Tela cheia responsiva** - Usa toda a área do navegador

## 🏆 Dicas

1. **Fique em movimento** - Parar é morrer!
2. **Colete os orbs verdes** - XP é essencial para evoluir
3. **Priorize upgrades de projétil** - Mais projéteis = mais kills
4. **Corra pros tesouros dourados** - Super poderes podem salvar sua vida
5. **Mantenha o combo** - Mate de perto para ganhar até 10x mais XP
6. **Cuidado com os bosses** - Eles dão muito mais dano!

## 🛠️ Tecnologias

- **HTML5 Canvas** - Renderização do jogo
- **CSS3** - Interface e efeitos visuais
- **JavaScript Vanilla** - Lógica do jogo
- **Web Audio API** - Sons procedurais
- **Gamepad API** - Suporte a controles

## 📁 Estrutura

```
neon-survivors.html  # Arquivo único com todo o jogo
README.md            # Este arquivo
```

## 🎨 Créditos

Desenvolvido com 💜 usando Claude (Anthropic)

---

**Divirta-se! 🎮**
